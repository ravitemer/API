<script>
	import {f7, Page, Navbar, NavTitle, Preloader, Block, Link, NavRight, NavTitleLarge, List, ListItem} from "framework7-svelte";
	...
	onMount(async () => {
		try {
			...
			f7.$('.page[data-name="Test"] .page-content').on("scroll", function (e, page) {
				x = this.scrollTop;
				f7.$("#svg").css({
					"margin-top": `${x * 0.5}px`,
				});
	...
	async function onClick(key) {
		...
		f7.emit("error", JSON.stringify(data, null, 2));
	}
	let x, y;

	const onPageInit = function (e, page) {
		x = page.$navbarEl;
		//y = page.$navbarEl.height()
		f7.emit("error", x);
	};

	const onPageBeforeIn = function (e, page) {
		x = page.$navbarEl;
		// y = page.$navbarEl.height()
		f7.emit("error", x);
	};
	const onPageBeforeOut = function (e, page) {
		x = page.$navbarEl;
		//y = page.$navbarEl.height()
		f7.emit("error", x);
	};
...
<Page bgColor={"green"} name={"Test"}>
	<Navbar class="testnav" transparent sliding={false}>
		<NavRight>
			<a class="">
				<i style="font-size : 22px; " class="f7-icons if-not-md"> arrow_down_circle_fill </i>
				<i style="font-size : 22px; " class="material-icons md-only"> arrow_down_circle_fill </i>
			</a>

			<a>
				<i style="font-size : 22px; " class="f7-icons if-not-md"> ellipsis_circle_fill </i>
				<i style="font-size : 22px; " class="material-icons md-only"> ellipsis_circle_fill </i>
			</a>
		</NavRight>
	</Navbar>

	<div style="" class="h-3/4 p-4 from-green-400 to-blue-500 flex space-y-4 flex-col justify-between inset-x-0">
		<!--margin-top:-100px;-->
		<!--Image-->
		<div class="h-40 z-0">
			<svg id="svg" style="" class="h-full w-full" xmlns="http://www.w3.org/2000/svg" data-name="Layer 1" viewBox="0 0 675.67004 631.94692" xmlns:xlink="http://www.w3.org/1999/xlink"
				><path
				...
				/><path
					...
					transform="translate(-262.16498 -134.02654)"
					fill="#ccc"
				/><circle cx="278.99773" cy="260.8659" r="215" fill="#e6e6e6" /><path
					...
					transform="translate(-262.16498 -134.02654)"
					fill="#fff"
				/><circle cx="278.99773" cy="260.8659" r="169" fill="#6c63ff" /><path
				...
				/><path
					...
					transform="translate(-262.16498 -134.02654)"
					fill="#fff"
				/><path d="M398.85271,342.81243l-22.24-9.03a5.49942,5.49942,0,0,0-4.14,10.18994l22.24,9.03a5.49945,5.49945,0,0,0,4.14-10.19Z" transform="translate(-262.16498 -134.02654)" fill="#fff" /><path
				...
				/><path
					...
					transform="translate(-262.16498 -134.02654)"
					fill="#fff"
				/><path d="M709.85271,445.81243l-22.24-9.03a5.49942,5.49942,0,0,0-4.14,10.18994l22.24,9.03a5.49945,5.49945,0,0,0,4.14-10.19Z" transform="translate(-262.16498 -134.02654)" fill="#fff" /><path
				...
				/><path
					...
					transform="translate(-262.16498 -134.02654)"
					fill="#3f3d56"
				/><circle cx="278.99773" cy="260.8659" r="13" fill="#ccc" /><path
				...
				/><path
					...
					transform="translate(-262.16498 -134.02654)"
					fill="#e6e6e6"
				/><circle cx="394.99773" cy="508.8659" r="17" fill="#e6e6e6" /><circle cx="406.99773" cy="73.8659" r="17" fill="#e6e6e6" /><circle cx="277.99773" cy="33.8659" r="17" fill="#e6e6e6" /><circle
					cx="154.99773"
					cy="71.8659"
					r="17"
					fill="#e6e6e6"
				/><circle cx="165.99773" cy="508.8659" r="17" fill="#e6e6e6" /><path
				...
				/><path
					...
					transform="translate(-262.16498 -134.02654)"
					fill="#3f3d56"
				/><polygon points="384.999 611.399 374.422 606.341 388.951 562.133 404.56 569.599 384.999 611.399" fill="#ffb7b7" /><path
				...
				/><path
					...
					transform="translate(-262.16498 -134.02654)"
					fill="#2f2e41"
				/><polygon points="458.703 593.34 459.434 595.189 458.703 595.189 458.703 593.34" fill="#2f2e41" /><path
				...
				/><path
					...
					fill="#3f3d56"
				/><path
					id="f4e08e8b-ef08-4d70-8ad3-b81af8a71ff9-13"
					...
					transform="translate(-262.16498 -134.02654)"
					fill="#2f2e41"
				/><circle cx="530.63215" cy="312.44125" r="23.33714" fill="#ffb7b7" /><path
		...
		<div class="flex text-color-black flex-none  flex-col items-center">
			<p class="text-lg mb-0.5 z-8 font-bold focus ">Pure Focus {x}</p>
			<p class="text-lg mb-px z-8 font-medium ">Apple Music Electronic</p>
			<p class="text-md">Pure Focus</p>
		</div>
		<!--Buttoms-->
		<div style="color:#f7f5ea;" class="flex flex-none mb-2 space-x-4 items-center">
			<button class="bg-gray-900 px-4 py-1 flex-1 font-medium text-lg rounded-md">Play</button>
			<button class="bg-gray-900 px-4 py-1 flex-1 font-medium text-lg rounded-md">Search</button>
		</div>
		<!--para-->
		<div class="text-sm flex-none text-color-black">
			Et ipsa commodi odit ea totam consequatur velit. Voluptate pariatur sit quam. <Link
				onClick={() => {
					try {
						f7.welcomescreen.check();
					} catch (err) {
						f7.emit("error", err.message);
					}
				}}
				class="text-color-primary">more</Link
			>
		</div>
	</div>

	<div style="overflow:hidden;margin-bottom:-200px;padding-bottom:200px;" class="rounded-t-xl z-10 p-4 break-all text-lg bg-gray-900 bottom-0 inset-x-0">
		Sed amet quod aspernatur <Link
			onClick={() => {
				try {
					//f7.welcomescreen.open();
					f7.contacts
						.select()
						.then((d) => f7.emit("error", JSON.stringify(d, null, 2)))
						.catch((err) => f7.emit("error", "Contacts Error:" + err.message));
				} catch (err) {
					f7.emit("error", err.message);
				}
			}}
			class="text-color-primary">more</Link
		> inventore. Nihil quidem dolorum atque voluptate voluptatibus. Ipsam aut et quia officiis quasi minus inventore. Ducimus repellat delectus consectetur porro sint unde sit. Consequatur illo cupiditate
		quibusdam enim voluptatem libero voluptatem. Itaque et et delectus reiciendis temporibus quo. Consectetur non rem impedit harum.Nostrum sequi perspiciatis saepe temporibus. Eum aut assumenda ut ipsum
		pariatur saepe amet. Ut molestias alias harum. Aut beatae modi rem provident. Quia quo neque qui qui nihil unde. Minima perspiciatis similique et voluptates. Odio delectus placeat vitae dolore sint.
		Cum voluptatibus eveniet illum necessitatibus dolores. Iste et harum facere. Non vitae labore expedita eius quod quod voluptas. Quisquam et voluptate velit consectetur cum.
	</div>
</Page>
